    ...
    .directive('kibanaPanel', function($compile) {
      ...
      var editorTemplate =
        ...
        '<div class="row-fluid panel-extra"><div class="panel-extra-container">' +
          ...
          '<span ng-repeat="task in panelMeta.modals" class="row-button extra" ng-show="panel.spyable">' +
          //p3#changes to include aria-label
            '<span bs-modal="task.partial"class="pointer"><a role="button" aria-label="Click here for panel information"> ' +
            '<i bs-tooltip="task.description" ng-class="task.icon" class="pointer"></i>' +
               '</a></span>' +
          '</span>' +
        ...
        '<span class="dropdown row-button extra" bs-tooltip="\'Export\'" data-placement="bottom" ng-show="panelMeta.exportfile">' +
            '<a title="Click here for export options" class="pointer" class="dropdown-toggle" data-toggle="dropdown">' +
                //'<i class="icon-save" class="pointer"></i>' +
          'Export' +
            '</a>' +
          ...
          '<h5>Number of Rows</h5><form name="myForm"><input type ="number" name ="exportcount" max="50000" min="0" ng-model="panel.exportSize"  ng-change="clear()"/>' +
          '<div ng-class="{ rederrors: myForm.exportcount.$error.max }">Maximum Limit for Export is 50000</div>'+
          '<!--input type="checkbox" ng-model="panel.exportAll" ng-checked="false"/> All Fields <tip>If this option is checked, all fields in the Solr schema will be exported. Otherwise, only the fields that you have selected to appear in your Table view will be exported</tip--></form>' +

          '<li>' +
          '<div ng-hide="myForm.exportcount.$error.max">'+
          ...
          '<span class="row-button extra" ng-show="panelMeta.loading == true">' +
            ...
            '</span>'+
          '</span>' +
          //p3#changes - Added sr-only to hide all headers.
          '<span class="row-button row-text panel-title" ng-show="panel.title" title=' + '{{panel.title}}' +'>'
          +'{{panel.title}}'  +
        '</span> ' +
