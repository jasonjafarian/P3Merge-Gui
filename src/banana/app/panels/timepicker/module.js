...
//        set_time_filter($scope.time);
	...
	var fromDate = $scope.timepicker.from.date;
	if ( fromDate instanceof Date ) {
		fromDate.setMinutes( fromDate.getMinutes() + fromDate.getTimezoneOffset() );
	}

	var toDate = $scope.timepicker.to.date;
	if ( toDate instanceof Date ) {
        ...
        from = $scope.panel.mode === 'relative' ? moment().subtract(timeShorthand,timeNumber) :
          moment(moment($scope.timepicker.from.date).format('MM/DD/YYYY') + " " + "00:00:00",'MM/DD/YYYY HH:mm:ss');
        // from = $scope.panel.mode === 'relative' ? moment(kbn.time_ago($scope.panel.timespan)) :
        //   moment(moment.utc($scope.timepicker.from.date).format('MM/DD/YYYY') + " " + "23:59:59",'MM/DD/YYYY HH:mm:ss');
        to = $scope.panel.mode !== 'absolute' ? moment() :
          moment(moment($scope.timepicker.to.date).format('MM/DD/YYYY') + " " + "23:59:59",'MM/DD/YYYY HH:mm:ss');
	...
	try {
		//google analytics code to send date range filter
		var dateRangeValue = " From = " + $scope.time.from.format('MM/DD/YYYY') 
						+ ", To = " + $scope.time.to.format( 'MM/DD/YYYY' );
		ga( 'send', 'event', { eventCategory:'Filter',
					eventAction : 'Date Range',
					eventLabel : dateRangeValue  } );
