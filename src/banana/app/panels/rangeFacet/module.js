...
function (angular, app, $, _, kbn, moment, timeSeries) {
  ...
  module.controller('rangeFacet', function($scope, $q, querySrv, dashboard, filterSrv, $timeout) {
    ...
    $scope.init = function() {
      ...
      $scope.get_data();
      // console.log(filterSrv.idsByTypeAndField('range',$scope.panel.range_field));
      var filters = filterSrv.idsByTypeAndField('range', $scope.panel.range_field);
      if (filters.length > 0) {
          $timeout(function(){}, 500);
          var filter = filterSrv.getRangeFilter();
          console.log("Range Facet Filter");
          console.log(filter);
          if (!filter) {
              console.log("Fall back");
              console.log(filterSrv.getByType("range", "active"));
              var filter = filterSrv.getByType("range", "active")[0];
              console.log(filter);
          }
          if (!filter) {
              console.log("Fall back2");
              var filter = filterSrv.list[filters[0]];
              console.log(filter);
          }
          $scope.set_configrations (filter.from, filter.to);
        $scope.close_edit();
        $scope.panel.showChart =  true;
      } else {
        $scope.panel.showChart =  false;
        ...
        // if($scope.query_id === query_id && _.difference(facetIds, $scope.panel.queries.ids).length === 0) {
          ...
          _.each($scope.panel.queries.ids, function(id,index) {
            ...
            // Entries from facet_ranges counts
              //Checking if entries if undefined
              if(typeof(results[index].facet_counts.facet_ranges[$scope.panel.range_field]) !== "undefined"){
...
//        scope.set_range_filter(scope.panel.minimum,scope.panel.maximum);
        ...
        // console.log("Rendering Range Facet Panel")
