...
function (angular, app, _, $, kbn) {
  ...
  module.controller('terms', function($scope, querySrv, dashboard, filterSrv) {
    ...
    //console.log("Panel object terms-module.js ==  " + JSON.stringify($scope.panel))
    //console.log("dashboard object terms-module.js ==  " + JSON.stringify(dashboard))
    ...
    $scope.build_search = function(term,negate) {
	 ...
	 //If there is manufacturer filter already defined with same manufacturer, remove from the filter
        var mfgFilterFound = false;
        _.each( filterSrv.list, function( v, k) {
           if ( v.type === 'terms') {
             if ( v.field === 'MANUFACTURER_NAME' && v.value === encodeURIComponent(term.label) ) {
               mfgFilterFound = true;
               filterSrv.remove(v.id);
               return true;
             }
           }
        } );
	 //if there is filter found for the manufacturer then include the filter
        if (!mfgFilterFound ) {
